  ...
  return (
    <div className="flex h-full w-full flex-col rounded-3xl border border-black/5 bg-white/40 p-4 shadow-2xl backdrop-blur-xl transition-colors duration-300 dark:border-white/10 dark:bg-white/5">
      <div className="mb-6 flex items-center justify-between px-2">
        ...
        <button className="flex h-8 w-8 items-center justify-center rounded-full bg-black/5 text-black hover:bg-black/10 transition-colors duration-300 dark:bg-white/10 dark:text-white dark:hover:bg-white/20">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto scrollbar-hide">
        {MOCK_TASKS.map((task, i) => (
          <motion.div
            ...
            className="group flex cursor-grab items-center gap-3 rounded-xl border border-black/5 bg-white/40 p-3 active:cursor-grabbing transition-colors duration-300 dark:border-white/5 dark:bg-white/5 dark:hover:bg-white/10"
          >
            <GripVertical className="h-4 w-4 text-black/20 opacity-0 transition-opacity group-hover:opacity-100 dark:text-white/20" />
            <div className="flex-1">
              <div className="text-sm font-medium text-black/90 transition-colors duration-300 dark:text-white/90">{task.title}</div>
              {task.tag && (
