...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ onClose, className = "" }) => {
  ...
  return (
    <div
      className={`relative flex h-full w-full flex-col overflow-hidden bg-white/40 backdrop-blur-xl dark:bg-black/40 ${className}`}
    >
      {/* Header */}
      <div className="flex shrink-0 items-center justify-between px-8 py-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold tracking-tight text-black/80 dark:text-white/80">
            Settings
          </h2>
        </div>
      </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <label className="text-xs font-bold tracking-wider text-black/40 uppercase dark:text-white/40">
                AI Provider
              </label>
                ...
                return (
                  <motion.div
                    ...
                    className={`relative cursor-pointer overflow-hidden rounded-2xl border transition-all duration-500 ${
                      isSelected
                        ? "border-black/5 bg-gradient-to-br from-white/80 to-white/40 shadow-xl ring-1 shadow-black/5 ring-black/5 dark:border-white/10 dark:from-white/10 dark:to-white/5 dark:shadow-black/20 dark:ring-white/10"
                        : "border-transparent bg-white/20 hover:bg-white/40 dark:bg-white/5 dark:hover:bg-white/10"
                    }`}
                  >
                    <div className="flex items-center gap-4 p-4">
                      <div
                        className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl shadow-sm transition-colors duration-300 ${
                          isSelected
                      ...
                      <div className="flex-1">
                        <h3
                          className={`text-[15px] font-semibold tracking-tight transition-colors duration-300 ${isSelected ? "text-black dark:text-white" : "text-black/60 dark:text-white/60"}`}
                        >
                          {p.name}
                        </h3>
                          ...
                          <div className="border-t border-black/5 bg-black/[0.02] p-4 dark:border-white/5 dark:bg-white/[0.02]">
                            ...
                            <div className="group relative">
                              <input
                                ...
                                className="w-full rounded-xl border border-transparent bg-black/5 px-4 py-3 text-sm font-medium text-black/80 transition-all outline-none placeholder:text-black/20 focus:border-purple-500/20 focus:bg-white focus:shadow-sm focus:ring-4 focus:ring-purple-500/10 dark:bg-white/5 dark:text-white/80 dark:placeholder:text-white/20 dark:focus:border-purple-400/20 dark:focus:bg-black/40 dark:focus:ring-purple-400/10"
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-medium tracking-wider text-black/30 uppercase dark:text-white/30">
                        Start
                      </label>
                      <div className="relative">
                        ...
                        />
                        <span className="absolute top-1/2 right-3 -translate-y-1/2 text-xs text-black/30 dark:text-white/30">
                          AM
                        </span>
                      </div>
                    </div>
          ...
          <div className="pt-4">
            ...
            >
              ...
              {/* Subtle glow effect */}
              <div className="absolute inset-0 -z-10 rounded-2xl bg-purple-500/20 opacity-0 blur-xl transition-opacity group-hover:opacity-100 dark:bg-purple-400/20" />
            </button>
          </div>
        </div>
      </div>
